...
def municipios_needing_update() -> list[str]:
    path = OUT / "resultados_municipios.csv"
...
def scrape(update_mode: bool = False):
    ...
    codes = []
    seen  = set()
    with open(SHARED / "puestos_master.csv") as f:
        for row in csv.DictReader(f):
    ...
    if update_mode and (OUT / "resultados_municipios.csv").exists():
        existing = []
        with open(OUT / "resultados_municipios.csv") as f:
    ...
    with open(OUT / "resultados_municipios.csv", "w", newline="") as f:
